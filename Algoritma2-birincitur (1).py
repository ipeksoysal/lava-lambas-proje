...
# ============================================================
# SABİT PARAMETRELER (Bulgular için)
# ============================================================
P1, P2, P3 = 269, 521, 839
P_FINAL = 1629  # final global mask için (şimdilik kullanılmıyor)
...
MIX_ITERS = 1  # Bulgular için 1 iterasyon

# ============================================================
# 0) BLOKLAMA / DURUM MATRİSİ
# ============================================================
...
# ============================================================
# 2) KATMAN: SİHİRLİ KARE PERMÜTASYONU
# ============================================================
...
# ============================================================
# Collatz maske üretimi (0/1 bit) — son 4 bit at + uzat
# ============================================================
...
def expand_collatz_mask(seed: int, length: int) -> np.ndarray:
    ...
    while len(out) < length:
        ...
        s = (3 * s + 1) % (2**31) + 2
        if len(out) == 0:
            out.extend([0, 1, 0, 1, 1, 0, 1, 0])  # fallback
    return np.array(out[:length], dtype=int)

# ============================================================
# 3) KATMAN: u,v TABANLI NONLİNEER KARIŞTIRMA
# ============================================================
...
# ============================================================
# 4) KATMAN: DİFÜZYON (invertible A üret + sütun bazlı)
# ============================================================
...
# ============================================================
# 5) KATMAN: TUR SONU BİT KARIŞTIRMA (Collatz XOR)
# ============================================================
def bytes_to_bits(byte_list: list[int]) -> np.ndarray:
    # MSB->LSB bit sırası (raporda tutarlı dursun)
    bits = []
    for b in byte_list:
        b = int(b) & 0xFF
...
def round_bit_mix(S: np.ndarray, seed: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    S (mod p) -> (S mod 256) byte'larına indir
    128 bit'e çevir
    Collatz maskesi üret (128 bit)
    XOR uygula
